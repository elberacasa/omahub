...
| Agents in the dock | Every Claude Code and Codex session gets a tile with a pixel pet in its project's color, read from the agent's own session record. The pet types in a rhythm while its agent works, hops with a squash and stretch when a turn ends, closes its eyes and dozes when it idles, stretches awake when work arrives, and bounces a red mark when it waits for you. Hover for the project, branch, tool, model, and last message. Click to jump to the agent, even inside Cursor, or right-click to answer it: send Codex a message, or allow or deny what Claude asks. Pick a pet for each company, how lively pets are, and when they nap |
