...
- For contributors filming Omahub: `dev/pet-stage` shows pets large in any look and mood with an end card, `dev/take --no-cursor` keeps the pointer out of a take while it still clicks, `dev/export-media --zoom` eases the camera between keyframes with pixel art kept crisp, and `dev/agent type --human` types at a person's pace.
