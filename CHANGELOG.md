...
- `agents/bar-limit` chooses which limit that is: Auto, the weekly limit unless a shorter one is nearly full, or Weekly, Session, Fullest, or any model limit a subscription reports, such as Fable Weekly. A limit that caps one model no longer stands for the whole plan on Auto.
