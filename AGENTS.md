...
Takes for the README or a post are staged with `dev/studio up`: generic projects in `~/Code` open in Cursor, with btop and a signed out X, while every real window waits on a hidden desktop. `dev/studio up --terminals` stages the same hidden desktop with no editor, for scenes filmed in terminals.

Agent pets are filmed without real agents. `dev/demos/agent-replica.py` draws a terminal like Claude Code or Codex from events a scene writes, and runs as a script named `claude` or `codex`, so the dock finds it; `dev/demos/pets-lib.sh` writes the session records the dock reads and deletes them after. `dev/pet-stage` shows pets large on the theme's background in any look and mood, switched on cue, with an end card. `dev/demos/pets-teaser.sh` films the teaser in one take. For a take that looks like a person at work, glide the pointer with `glide` from `dev/demo-lib.sh`, type with `dev/agent type <text> --human`, record with `dev/take <name> <seconds> --no-cursor` to keep the pointer out, and ease the camera in the export with `dev/export-media <take> <name> <start-end> --zoom time:x:y:scale,...`. `dev/studio down` puts everything back and deletes only the projects it made. Every export runs `dev/privacy-check`, which reads the frames for user and host names, the home folder, git identity, and local network addresses, and still look at every frame before posting.
